...
resource "aws_vpc" "lunx_vpc" {
  ...
  tags = {
    Name = "lunx_vpc-${random_integer.random.id}"
  }
  lifecycle {
...
resource "aws_subnet" "lunx_public_subnet" {
  ...
  tags = {
    Name = "lunx_public_${count.index + 1}"
  }
}
...
resource "aws_subnet" "lunx_private_subnet" {
  ...
  tags = {
    Name = "lunx_private_${count.index + 1}"
  }
}

resource "aws_db_subnet_group" "lunx_rds_subnetgroup" {
  count      = var.db_subnet_group == "true" ? 1 : 0
  name       = "lunx_rds_subnetgroup"
  subnet_ids = aws_subnet.lunx_private_subnet.*.id
  tags = {
    Name = "lunx_rds_sng"
  }
}
...
resource "aws_default_route_table" "lunx_private_rt" {
  default_route_table_id = aws_vpc.lunx_vpc.default_route_table_id
...
resource "aws_route_table_association" "lunx_public_assoc" {
  count          = var.public_sn_count
  subnet_id      = aws_subnet.lunx_public_subnet.*.id[count.index]
  route_table_id = aws_route_table.lunx_public_rt.id
}
